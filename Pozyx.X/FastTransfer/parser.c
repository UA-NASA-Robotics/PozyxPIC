...
void Parser_init (struct ring_buffer_t* buf, uint16_t* arr, bool* flags, int arr_size, uint8_t address)
{
	m_buf = buf;
	m_arr = arr;
	m_flags = flags;
...
void Parser_first_byte ()
{
	...
	if (m_lookahead >= rbuffer_size(m_buf))
	{
	...
	if (rbuffer_get(m_buf, m_lookahead) != 0x06)
	{
...
void Parser_second_byte ()
{
	...
	if (rbuffer_get(m_buf, m_lookahead) != 0x85)
	{
...
void Parser_sender()
{
	...
	m_sender = rbuffer_get(m_buf, m_lookahead);
	++m_lookahead;
}
...
void Parser_receiver ()
{
	...
	if (rbuffer_get(m_buf, m_lookahead) != m_address)
	{
...
void Parser_message_length ()
{
	...
	m_length = rbuffer_get(m_buf, m_lookahead);
...
{
	// verify that there is enough data in the buffer to continue parsing:
	if (m_lookahead + m_length - 1 >= rbuffer_size(m_buf))
	{
...
void Parser_write_message_data ()
{
	...
	{
		uint8_t index = rbuffer_get(m_buf, i);
		uint16_t lsb = rbuffer_get(m_buf, i + 1);
		uint16_t msb = rbuffer_get(m_buf, i + 2);
